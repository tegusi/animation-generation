...
class AudioHandler:
    ...
    def convert_to_deepspeech(self, audio, fps=25):
        ...
        with tf.Session(graph=graph) as sess:
            for subj in audio.keys():
                for seq in audio[subj].keys():
                    ...
                    num_frames = int(round(audio_len_s * fps))
                    network_output = interpolate_features(network_output[:, 0], 50, fps,
