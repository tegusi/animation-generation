...
class VOCAModel(BaseModel):
    ...
    def _render_helper(self, out_folder, data_specifier):
        ...
        if data_specifier == 'training':
            ...
            num_training_subjects = self.batcher.get_num_training_subjects()
            condition_subj_idx = [range(1) for idx in subject_idx]
        else:
    ...
    def _render_sequences_helper(self, video_fname, seq_raw_audio, seq_processed_audio, seq_template, seq_verts, condition_idx):
        ...
        if int(cv2.__version__[0]) < 3:
            print('cv2 < 3')
            writer = cv2.VideoWriter(tmp_video_file.name, cv2.cv.CV_FOURCC(*'mp4v'), 25, (1600, 800), True)
        else:
            print('cv2 >= 3')
            writer = cv2.VideoWriter(tmp_video_file.name, cv2.VideoWriter_fourcc(*'mp4v'), 25, (1600, 800), True)
        ...
        cmd = ('ffmpeg' + ' -i {0} -i {1} -vcodec h264 -ac 2 -strict -2 -channel_layout stereo -pix_fmt yuv420p  {2}'.format(
